<script setup>
import { ref, computed, watch, onMounted, onUnmounted } from 'vue' // onMounted, onUnmounted qo'shildi
...
// YANGI: Dropdownni ushlab turish uchun ref
const pageSizeMenuRef = ref(null)

// --- CLICK OUTSIDE LOGIC (YANGI) ---
const handleClickOutside = (event) => {
  // Agar menu ochiq bo'lsa VA bosilgan joy bizning menu ichida bo'lmasa -> Yopamiz
  if (showPageSizeMenu.value && pageSizeMenuRef.value && !pageSizeMenuRef.value.contains(event.target)) {
    showPageSizeMenu.value = false
  }
}

// Sahifa yuklanganda hodisani qo'shamiz
onMounted(() => {
  document.addEventListener('click', handleClickOutside)
})

// Sahifadan chiqib ketganda hodisani o'chiramiz (xotirani tozalash uchun)
onUnmounted(() => {
...
<template>
  <div class="flex flex-col h-full bg-white dark:bg-slate-800 border border-indigo-100 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden relative z-0">
    ...
    <div class="flex-1 overflow-auto custom-scrollbar relative">
      <table class="w-full text-left border-collapse" :style="{ minWidth: '100%' }">
        <thead class="bg-indigo-50 dark:bg-slate-900 sticky top-0 z-[50] text-[12px] text-center items-center font-bold text-indigo-900 dark:text-indigo-300 uppercase tracking-wide shadow-sm">
          <tr>
            <th v-if="showIndex" class="th-fixed left-0 text-center w-12 z-[60]" @mouseenter="hoveredColumn = 'index'" @mouseleave="hoveredColumn = null">№</th>
    ...
    <div v-if="paginatedData.length > 0" class="flex-none bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-4 flex flex-wrap items-center justify-between gap-4 z-40 relative shadow-sm transition-colors duration-300">
      
      <div class="flex items-center gap-4 text-xs font-medium text-slate-500 dark:text-slate-400">
        <div class="flex items-center gap-2">
          <span class="bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/20 px-2.5 py-1 rounded-md font-bold text-sm transition-colors">
            {{ props.items.length }}
          </span>
          <span class="text-slate-600 dark:text-slate-400">jami</span>
        </div>
        ...
        <div class="relative" ref="pageSizeMenuRef">
          ...
          <transition name="dropdown-up">
            <div v-if="showPageSizeMenu" class="absolute bottom-full left-0 mb-2 w-32 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden p-1 z-50 ring-1 ring-black/5 dark:ring-white/5">
              <button 
      ...
      <div class="flex items-center gap-2 select-none">
        <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1" class="h-8 w-8 flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-slate-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white disabled:dark:hover:bg-slate-800 active:scale-95 shadow-sm">
          <i class="fa-solid fa-chevron-left text-xs"></i>
        </button>
        <div class="flex items-center gap-1">
          <button v-for="p in totalPages" :key="p" @click="changePage(p)" class="w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-all" :class="currentPage === p ? 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-md shadow-indigo-500/30 dark:shadow-indigo-900/40 scale-105' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-indigo-600 dark:hover:text-indigo-300'" v-show="p === 1 || p === totalPages || (p >= currentPage - 1 && p <= currentPage + 1)">
            {{ p }}
          </button>
        </div>
        <button @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages" class="h-8 w-9 flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-slate-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white disabled:dark:hover:bg-slate-800 active:scale-95 shadow-sm">
