...
const { 
    sessions, 
    activeSessionId,
    document: inboundDocument, // 'documents' nomi chalkashlik yaratmasligi uchun o'zgartirildi
    isSubmitting, 
    totalSum, 
...
// Template'dagi ref="elementName" bilan bog'lanish uchun:
...
const selectBranch = (b) => { 
    inputStore.document.branchId = b.id; 
...
const changeQty = (item, delta) => { 
    ...
    if (newQty > 0) {
        // DIQQAT: item._id emas, item.productId ishlatilishi aniqroq (Store'ga qarab)
  ...
  <div class="h-screen w-full bg-[#F0FDFA] dark:bg-[#020617] flex flex-col font-sans overflow-hidden text-slate-600 dark:text-slate-400 selection:bg-teal-500 selection:text-white transition-colors duration-300">
    ...
    <div class="flex-1 flex overflow-hidden relative z-0">
      ...
      >
        <div class="px-5 py-4 sticky top-0 z-30 bg-[#F0FDFA]/95 dark:bg-[#020617]/95 backdrop-blur-md border-b border-teal-100/50 dark:border-slate-800/50 flex gap-3">
           <div class="relative group flex-1">
             ...
             <input v-model="productSearch" type="text" placeholder="Mahsulot qidirish..." class="block w-full h-11 pl-10 pr-4 bg-white dark:bg-[#0F172A] border border-teal-100 dark:border-slate-700 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all text-slate-800 dark:text-white">
           </div>
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
            <div v-for="product in filteredProducts" :key="product._id" @click="addToInbound(product)" class="group relative bg-white dark:bg-[#0F172A] rounded-2xl p-2 border border-teal-50 dark:border-slate-800 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col overflow-hidden" :class="{'ring-2 ring-teal-500 border-teal-500': isAdded(product)}">
              <div class="aspect-[4/3] bg-slate-100 dark:bg-slate-900 rounded-xl overflow-hidden relative mb-2">
                 ...
                 <transition name="pop">
                    <div v-if="isAdded(product)" class="absolute inset-0 bg-teal-500/10 flex items-center justify-center backdrop-blur-[1px]">
                        <div class="w-10 h-10 bg-white dark:bg-teal-500 rounded-full flex items-center justify-center text-teal-600 dark:text-white shadow-xl scale-110 border-2 border-white"><i class="fa-solid fa-check"></i></div>
                    </div>
                 </transition>
              </div>
              <div class="px-1 flex-1 flex flex-col">
                 ...
                 <div class="mt-auto border-t border-slate-100 dark:border-slate-800 pt-2 flex justify-between items-center text-[10px]">
                     <span class="text-slate-400">Oxirgi narx:</span>
      ...
      >
        <div class="shrink-0 bg-slate-50 dark:bg-[#0B1120] border-b border-slate-200 px-3 pt-3 flex items-end gap-1 overflow-x-auto no-scrollbar">
          <button @click="mobileTab = 'catalog'" class="lg:hidden w-10 h-10 mb-1 rounded-lg bg-white dark:bg-slate-800 border flex items-center justify-center text-slate-500 shadow-sm"><i class="fa-solid fa-arrow-left"></i></button>
          <button v-for="session in sessions" :key="session.id" @click="activeSessionId = session.id" class="relative px-4 py-3 rounded-t-xl text-[11px] font-bold transition-all min-w-[110px] flex items-center justify-between border-t border-x" :class="activeSessionId === session.id ? 'bg-white dark:bg-[#020617] text-teal-600 border-slate-200 dark:border-slate-800 !border-b-transparent z-10 shadow-sm' : 'text-slate-500 border-transparent'">
            <span>{{ session.name }}</span>
            <span v-if="sessions.length > 1" @click.stop="removeSession(session.id)" class="hover:text-red-500 transition ml-2"><i class="fa-solid fa-xmark"></i></span>
          </button>
          <button @click="addSession" class="w-10 h-10 mb-1 flex items-center justify-center rounded-lg hover:bg-teal-50 text-slate-400 hover:text-teal-600 transition"><i class="fa-solid fa-plus"></i></button>
        </div>

        <div class="shrink-0 p-4 bg-white dark:bg-[#0F172A] border-b border-slate-100 dark:border-slate-800 shadow-sm">
            <div class="grid grid-cols-2 gap-3">
                <div class="relative" ref="supplierDropdownRef">
                    ...
                    <button @click="showSupplierList = !showSupplierList; showBranchList = false" class="w-full flex items-center justify-between p-2.5 bg-slate-50 dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 rounded-xl hover:border-teal-400 transition shadow-sm" :class="{'border-teal-500': showSupplierList}">
                        <div class="flex items-center gap-2 overflow-hidden">
                            <i class="fa-solid fa-truck-fast text-orange-500 text-xs"></i>
                            <span class="text-[10px] font-black truncate" :class="activeSupplier ? 'text-slate-800 dark:text-white' : 'text-rose-500'">{{ activeSupplier ? activeSupplier.company : 'Tanlang' }}</span>
                        </div>
                        <i class="fa-solid fa-chevron-down text-[8px] text-slate-400"></i>
                    </button>
                    <transition name="dropdown">
                        <div v-if="showSupplierList" class="absolute top-14 left-0 w-full bg-white dark:bg-[#1E293B] rounded-xl shadow-2xl border border-slate-200 z-50 p-1 overflow-hidden">
                ...
                <div class="relative" ref="branchDropdownRef">
                    <label class="block text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Zavod filyali</label>
                    <button @click="showBranchList = !showBranchList; showSupplierList = false" class="w-full flex items-center justify-between p-2.5 bg-slate-50 dark:bg-[#1E293B] border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-400 transition shadow-sm" :class="{'border-indigo-500': showBranchList}">
                        <div class="flex items-center gap-2 overflow-hidden">
                            <i class="fa-solid fa-warehouse text-indigo-500 text-xs"></i>
                            <span class="text-[10px] font-black truncate" :class="activeBranch ? 'text-slate-800 dark:text-white' : 'text-rose-500'">{{ activeBranch ? activeBranch.company : 'Tanlang' }}</span>
                        </div>
                        <i class="fa-solid fa-chevron-down text-[8px] text-slate-400"></i>
                    </button>
                    <transition name="dropdown">
                        <div v-if="showBranchList" class="absolute top-14 left-0 w-full bg-white dark:bg-[#1E293B] rounded-xl shadow-2xl border border-slate-200 z-50 p-1">
        ...
        <div class="flex-1 overflow-y-auto p-3 bg-slate-50/50 dark:bg-[#020617] custom-scroll-teal pb-32 lg:pb-3">
          <div v-if="activeDocument.items.length === 0" class="h-full flex flex-col items-center justify-center opacity-40 select-none">
              ...
              <p class="text-sm font-bold">Ro'yxat bo'sh</p>
          </div>
              ...
              <div class="grid grid-cols-12 gap-2">
                  ...
                  <div class="col-span-4">
                      ...
                      <div class="flex items-center h-9 bg-slate-100 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
                          ...
                          <input v-model.number="item.qty" type="number" class="flex-1 w-full bg-transparent text-center text-xs font-black outline-none dark:text-white">
        ...
        <div class="shrink-0 p-4 bg-white dark:bg-[#0F172A] border-t border-slate-200 dark:border-slate-700 z-30 pb-24 lg:pb-4 shadow-[-5px_0_15px_rgba(0,0,0,0.05)]">
            <div class="flex justify-between items-center mb-4">
                ...
                <span class="text-xl font-black font-mono tracking-tight text-teal-600">{{ formatPrice(totalSum) }}</span>
            </div>
            <button 
    ...
    <div class="lg:hidden fixed bottom-0 left-0 w-full bg-white dark:bg-[#0F172A] border-t border-slate-200 flex z-50">
      ...
      <button @click="mobileTab = 'inbound'" class="flex-1 py-3 flex flex-col items-center gap-1 border-t-2 transition-all relative" :class="mobileTab === 'inbound' ? 'border-teal-600 text-teal-600 bg-teal-50/20' : 'border-transparent text-slate-400'">
        ...
        <span v-if="activeDocument.items.length" class="absolute top-2 right-1/3 w-2 h-2 bg-rose-500 rounded-full animate-pulse"></span>
      </button>
    </div>
