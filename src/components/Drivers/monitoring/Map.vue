  ...
  <div>
    ...
    >
      <i
        ...
        class="fa-solid fa-earth-africa fa-lg"
      ></i>
      <i v-if="isSatellite === true" class="fa-solid fa-map fa-lg"></i>
    </div>
...
const { onlineUsers, onlineDrivers } = storeToRefs(socketStore);
...
const drivers = ref([
  {
    ...
    fullname: "Vali Jo‘rayev",
    age: "28",
    gender: "Erkak",
    position: "2",
    orders: "34",
    car_name: "Damas",
    car_number: "80 S 936 AW",
    car_color: "oq",
    lat: 40.1006,
    lng: 64.6834,
    address: "G‘ijduvon, Buxoro viloyati",
  },
  {
    id: 2,
    avatar:
      "https://img.freepik.com/premium-photo/happy-female-driving-car-smiling-camera-posing-auto_116547-25551.jpg?semt=ais_hybrid&w=740",
...
onMounted(() => {
  // Real haydovchi joylashuvini olish uchun socket orqali so‘rov yuboramiz
  if (navigator.geolocation && user && token && user.role === 1000) {
    navigator.geolocation.watchPosition(
      (position) => {
        const lat = position.coords.latitude;
        const lng = position.coords.longitude;
        // Socket orqali backendga jo‘natamiz
        socketStore.DriverLocation({
          ...user,
          lat,
          lng,
        });
    ...
    marker.bindPopup(
      `
        <div class="custom-popup">
          ...
          <img src="${driver.username}" alt="Avatar" style=""width: 100%; height: 200px; border-radius: 10%; object-fit: cover; margin-bottom: 10px;">
          <div class=" mt-2">
          <p class="text-center text-[16px] font-semibold ">${driver.username}</p>
          <div class=" text-[12px] flex gap-2 ">
           <p class="">Yoshi: ${driver.username}</p>
           <p class="">Jinsi: ${driver.username}</p>
           <p class="">Zakazlar: ${driver.username}</p>
           </div>
           </div>
        <div class="mt-3">
          <p class="text-center items-center font-semibold text-[13px]
          ">Mashina ma'lumotlari:</p>
           <p>Mashina nomi: ${driver.username}</p>
           <p> Mashina raqami: ${driver.username}</p>
          <p> Mashina rangi: ${driver.username} </p>

       <p>Joriy manzili :  ${driver.username}</p>
