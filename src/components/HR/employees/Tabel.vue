...
const columns = [
  ...
  { key: 'registeredAt', label: 'Sana', width: '140px', align: 'center', sortable: true },
...
// Modal Actions
const openAddModal = () => {
  store.AddEmployeeModal(); // Store dagi funksiya
};

// Export Handler
const handleExport = (type) => {
  ElMessage.success(`${type.toUpperCase()} yuklanmoqda...`);
  store.ExportExcel({ type }); // Store funksiyasi
};
...
const formatDate = (date) => date ? moment.utc(date).tz("Asia/Tashkent").format("DD.MM.YYYY") : "-";
    ...
    >
      <template #fullname="{ row }">
        <div class="flex items-center gap-3">
           <div class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-500 text-xs font-bold uppercase border border-white shadow-sm ring-2 ring-slate-50 dark:ring-slate-800">
             {{ row.fullname ? row.fullname.charAt(0) : 'U' }}
           </div>
           <div class="flex flex-col">
             <span class="font-semibold text-slate-700 dark:text-slate-200 text-sm hover:text-indigo-600 cursor-pointer transition-colors" @click="handleAction('view', row)">
               {{ row.fullname }}
             </span>
             <span class="text-[10px] text-slate-400">ID: {{ row._id ? row._id.slice(-6).toUpperCase() : '---' }}</span>
           </div>
        </div>
      </template>

      <template #department="{ row }">
         <span class="text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded">{{ row.department }}</span>
      </template>
      ...
      <template #address="{ row }">
         <div class="text-xs text-slate-500 truncate max-w-[180px]" :title="row.address">
            {{ row.address }}
         </div>
      </template>

      <template #registeredAt="{ row }">
         <span class="text-xs font-mono text-slate-400">{{ formatDate(row.registeredAt) }}</span>
      </template>
      ...
      <template #status="{ row }">
        <span class="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase border shadow-sm flex items-center justify-center gap-1.5 w-fit mx-auto"
          ...
          <i :class="row.status === 'active' || row.status === 'Online' ? 'fa-solid fa-circle-check' : 'fa-solid fa-circle-pause'"></i>
          {{ row.status === 'active' || row.status === 'Online' ? 'Aktiv' : 'Nofaol' }}
        </span>
      </template>
