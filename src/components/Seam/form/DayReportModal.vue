  ...
  <div>
    ...
    <el-dialog v-model="reports.is_modal" width="1200">
      <div class="bg-white p-1 text-[10px] rounded shadow">
        ...
        >
          <div>Partya nomeri: {{ reports.form.party_number }}</div>
          <div>Buyurtmachi: {{ reports.form.customer_name }}</div>
          <div>Artikul: {{ reports.form.artikul }}</div>
        </div>
        <el-form
          :model="model"
          ref="formRef"
          ...
          size="small"
          label-position="top"
          class="filter-box md:grid md:grid-cols-12 sm:flex sm:flex-wrap gap-1"
        >
